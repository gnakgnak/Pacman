...
static bool pacman_movable(Pacman* pacman, Map* M, Directions targetDirec) {
	...
	{
	case UP:
            if(is_room_block(M,x,y-1 )==1||is_wall_block(M, x,y-1 )==1)return false;
            else return true;
	case DOWN:
            if(is_room_block(M, x,y+1)==1||is_wall_block(M, x,y+1)==1)return false;
            else return true;
	case LEFT:
            if(is_room_block(M, x-1, y)==1||is_wall_block(M, x-1,y)==1)return false;
            else return true;
	case RIGHT:
            if(is_room_block(M, x+1, y)==1||is_wall_block(M, x+1, y)==1)return false;
            else return true;
...
void pacman_draw(Pacman* pman) {
	...
	if (game_over) {
        ALLEGRO_TIMER* time=pman->death_anim_counter;
        if(!al_get_timer_started(time))al_start_timer(time);
            al_draw_scaled_bitmap(pman->die_sprite, (al_get_timer_count(time)/10)*16,0,16, 16, drawArea.x+fix_draw_pixel_offset_x, drawArea.y+fix_draw_pixel_offset_y, draw_region, draw_region, 0);
        if (al_get_timer_count(time)/10==12) {
            al_rest(1);
			...
			{
			case LEFT:
                    if(pman->objData.moveCD%4<2)al_draw_scaled_bitmap(pman->move_sprite, 32, 0,16, 16,drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,draw_region, draw_region, 0);
                    else al_draw_scaled_bitmap(pman->move_sprite, 48, 0, 16, 16, drawArea.x+fix_draw_pixel_offset_x, drawArea.y+fix_draw_pixel_offset_y, draw_region, draw_region, 0);
                    break;
            case RIGHT:
                    if(pman->objData.moveCD%4<2)al_draw_scaled_bitmap(pman->move_sprite, 0, 0,16, 16,drawArea.x + fix_draw_pixel_offset_x, drawArea.y+fix_draw_pixel_offset_y,draw_region, draw_region, 0);
                    else al_draw_scaled_bitmap(pman->move_sprite, 16, 0, 16, 16, drawArea.x+fix_draw_pixel_offset_x, drawArea.y+fix_draw_pixel_offset_y, draw_region, draw_region, 0);
                    break;
            case UP:
                    if(pman->objData.moveCD%4<2) al_draw_scaled_bitmap(pman->move_sprite, 64, 0, 16, 16, drawArea.x+fix_draw_pixel_offset_x, drawArea.y+fix_draw_pixel_offset_y, draw_region, draw_region, 0);
                    else al_draw_scaled_bitmap(pman->move_sprite, 80, 0, 16, 16, drawArea.x+fix_draw_pixel_offset_x, drawArea.y+fix_draw_pixel_offset_y, draw_region, draw_region, 0);
                    break;
                case DOWN :
                    if(pman->objData.moveCD%4<2)al_draw_scaled_bitmap(pman->move_sprite, 96, 0, 16, 16, drawArea.x+fix_draw_pixel_offset_x, drawArea.y+fix_draw_pixel_offset_x, draw_region, draw_region, 0);
                    else al_draw_scaled_bitmap(pman->move_sprite, 112, 0, 16, 16, drawArea.x+fix_draw_pixel_offset_x, drawArea.y+fix_draw_pixel_offset_y, draw_region, draw_region, 0);
                    break;
            default:
