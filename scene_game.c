...
int stage;
char maptxt[2][25]= {"Assets/map_nthu.txt","Assets/emily.txt"};
/* Internal variables*/
...
static void init(void) {
	...
	game_main_Score = 0;
	// create map
    basic_map = create_map(maptxt[stage]);
	// [TODO]
...
static void checkItem(void) {
	...
	case '.':
        basic_map->beansNum--;
        game_main_Score+=10;
//            game_log("score = %d",game_main_Score);
...
static void status_update(void) {
	...
	for (int i = 0; i < GHOST_NUM; i++) {
		if (ghosts[i]->status == GO_IN)
            continue;
        if (ghosts[i]->status == FREEDOM) ghosts[i]->status = 2;
        if( ! cheat_mode && ghosts[i]->status == FLEE && RecAreaOverlap( getDrawArea(pman->objData, GAME_TICK_CD),getDrawArea(ghosts[i]->objData, GAME_TICK_CD)))
        {
...
static void draw(void) {
    ...
    sprintf(score,"%010.d",game_main_Score);
	draw_map(basic_map);
//    ALLEGRO_FONT* front=load_font("Minecraft.ttf", 20);
    al_draw_text(menuFont,al_map_rgb(254, 144, 136) , 666 , 0, ALLEGRO_ALIGN_CENTRE ,  score );
	pacman_draw(pman);
	if (game_over)
		return;
