...
static ALLEGRO_BITMAP *junghook;
...
static void init() {
    ...
    junghook = load_bitmap("Assets/junghook.png"); //not finish yet
...
static void draw() {
	...
	//draw title image
	al_draw_scaled_bitmap(gameTitle,0, 0,gameTitleW, gameTitleH,offset_w, offset_h,gameTitleW * scale, gameTitleH * scale,0);
	al_draw_text(menuFont,al_map_rgb(255, 255, 255),SCREEN_W/2,SCREEN_H - 150,ALLEGRO_ALIGN_CENTER,"PRESS \"ENTER\"");
    al_draw_scaled_bitmap(junghook, 0, 0, al_get_bitmap_width(junghook), al_get_bitmap_height(junghook),0 , 500, al_get_bitmap_width(junghook)*0.4, al_get_bitmap_height(junghook)*0.4, 0);
