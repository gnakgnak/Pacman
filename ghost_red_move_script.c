...
static void ghost_red_move_script_FREEDOMshortest(Ghost* ghost, Map* M){
	// [HACKATHON 2-4]
...
//    Directions shortestpath = shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y, pacman->objData.Coord.x, pacman->objData.Coord.y);
    srand(time(NULL));
    do   nexttmp=proba[rand()%cnt];
    while((next==nexttmp &&cnt!=1 ));//||(nexttmp != shortestpath)
...
void ghost_red_move_script(Ghost* ghost, Map* M, Pacman* pacman) {
	if (!movetime(ghost->speed))
		...
		case BLOCKED:
			...
			break;
		case FREEDOM:
			ghost_red_move_script_FREEDOMshortest(ghost, M);
			break;
		case GO_OUT:
